  ...
  async parse(url: string): Promise<ParsedMangaData> {
    try {
      ...
      const absoluteCoverUrl = coverUrl
        ? new URL(coverUrl, 'https://mangabuff.ru').href
        : undefined;
  ...
  private async extractChapters(
    ...
    if (mangaId) {
      ...
      while (hasMoreChapters) {
        if (offset > 50000) throw new Error('Превышен лимит глав источника');
        try {
          ...
          throw new Error('Не удалось загрузить полный список глав MangaBuff');
        }
      }
      ...
      if (!offsetEverReturnedChapters) {
        ...
        while (pageHasMore) {
          if (page > 1000) throw new Error('Превышен лимит страниц списка');
          try {
            const response = await session.post(
              'https://mangabuff.ru/chapters/load',
